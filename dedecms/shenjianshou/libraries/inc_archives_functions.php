...
$backurl = preg_match("#content_#", $backurl) ? "<a href='$backurl'>[<u>".$dede_charset['tpl']['menu_list']."</u>]</a> &nbsp;" : '';
 ...
 * @return    bool
 */
function CheckChannel($typeid, $channelid, $dsql = null)
{
...
{
    global $cuserLogin,$cfg_ddimg_width,$cfg_ddimg_height,$cfg_basedir,$ddcfg_image_dir,$cfg_addon_savetype,$dede_charset;
    $ntime = time();
     ...
     !empty($_FILES[$litpic]['tmp_name']) && is_uploaded_file($_FILES[$litpic]['tmp_name']))
    {
        ...
        {
            ta_fail(TA_ERROR_PLUGIN_ERROR, "image form is error", $dede_charset['msg']['fail_upload_image']);
            exit();
        }
...
{
    global $cuserLogin,$cfg_basedir,$cfg_image_dir,$title, $dsql,$dede_charset;
    if($ntitle!='')
    {
        $title = $ntitle;
    }
...
{
    global $arcID, $typeid, $cfg_make_andcat, $cfg_makeindex, $cfg_make_prenext,$dede_charset;
    $revalue = $dolist = '';
    if($cfg_makeindex=='Y' || $cfg_make_andcat=='Y' || $cfg_make_prenext=='Y')
    {
        ...
        $revalue = "<table width='80%' style='border:1px dashed #cdcdcd;margin-left:20px;margin-bottom:15px' id='tgtable' align='left'><tr><td bgcolor='#EBF5C9'>&nbsp;<strong>".$dede_charset['updateing']."：</strong>\r\n</td></tr>\r\n";
